...
class BeamSearch(object):
    ...
    def __init__(self, beam_size, max_len, pad):
        ...
        self.best_finished_score = float('-inf')  # Best score of completed hypotheses
        self._counter = count()  # For ordering nodes with the same score
        self.completed_nodes = 0  # Track how many hypotheses are completed
    ...
    def add_final(self, score, node):
        ...
        # Update best finished score if this hypothesis is better
        if -score > self.best_finished_score:
            self.best_finished_score = -score
    ...
    def prune(self):
        """Prunes incomplete hypotheses worse than the best finished hypothesis and keeps beam_size."""
        nodes = PriorityQueue()
        # Only prune based on best_finished_score if we have completed nodes
        # If no nodes have finished yet (best_finished_score = -inf), we won't prune by score
        while not self.nodes.empty():
            score, _, node = self.nodes.get()
            # If we have a best finished score, prune hypotheses that are worse
            if self.best_finished_score == float('-inf') or (-score >= self.best_finished_score):
                nodes.put((score, next(self._counter), node))

        # Keep the beam size constant
        pruned_nodes = PriorityQueue()
        for _ in range(min(self.beam_size, nodes.qsize())):
            pruned_nodes.put(nodes.get())
